...
const Dashboard = () => {
        ...
        try {
            ...
            if (data?.event_id) {
                ...
                localStorage.setItem(
                    'pekan_active_event',
                    JSON.stringify({ id: data.event_id, nama: data.nama_event || null })
                );
            } else {
    ...
    useEffect(() => {
        ...
        if (supabaseRealtime) {
            channel = supabaseRealtime
                .channel('dashboard-kunjungan-compact')
                .on(
                    'postgres_changes',
    ...
    const processScannerTap = useCallback(
        async (rawUid) => {
            ...
            if (!activeEventIdRef.current) {
                ...
                setScannerResult({
                    ok: false,
                    message: 'Tidak ada event aktif. Aktifkan event terlebih dahulu.',
                    uid: scannedUid,
                });
                toast.warning('Tidak ada event aktif. Tap member belum bisa diproses.');
                setTimeout(() => setScannerState('idle'), 2200);
                return;
            }
            ...
            try {
                ...
                if (!response.ok || payload?.status !== 'success') {
                    ...
                    setScannerState('error');
                    setScannerResult({ ok: false, message, uid: scannedUid });
                    toast.error(message);
                    return;
                }
                ...
                setScannerResult({ ok: true, uid: scannedUid, ...payload.data });
                toast.success(
                    aksi === 'keluar'
    ...
    const handleScannerChange = useCallback((e) => {
    ...
    return (
        <div className="space-y-5 font-sans pb-1">
            <style>{`
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {statCards.map((item) => {
                    ...
                    return (
                        <div key={item.key} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                            ...
                            <div className={`text-3xl md:text-[32px] font-bold leading-none ${item.valueTone}`}>
                                {isLoadingStats ? '...' : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[360px,minmax(0,1fr)] gap-5 lg:items-stretch">
                <div className="space-y-4">
                    ...
                    <div
                        className={`p-4 rounded-2xl shadow-sm border transition-colors ${scannerTone}`}
                        onClick={() => scannerInputRef.current?.focus()}
                    >
                        <div className="flex items-start justify-between gap-3 mb-3">
                            <div>
                                <h3 className="text-base font-bold text-gray-800 mb-1">Tap Member NFC</h3>
                                <p className="text-xs text-gray-500 leading-relaxed">
                                    Reader R20C-USB akan terbaca otomatis di sini. Area ini sengaja dibuat lebih ringkas agar dashboard tidak memakan satu layar penuh.
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-[1.2fr,.8fr] gap-3">
                            <div className="rounded-2xl border border-dashed border-current/10 bg-white/80 px-4 py-4">
                                <div className="text-[11px] uppercase tracking-[0.18em] text-gray-400 font-semibold mb-2">UID Terbaca</div>
                                <div className="text-xl md:text-2xl font-bold tracking-[0.16em] text-gray-800 min-h-[30px] font-mono break-all">
                                    {scannerState === 'scanning' ? scannerBuffer || '…' : scannerResult?.uid || '—'}
                                </div>
                                <div
                                    className={`mt-2 text-sm font-medium ${
                                        scannerState === 'error'
                                            ? 'text-red-600'
                                            : scannerState.startsWith('success')
                                              ? 'text-green-700'
                                              : scannerState === 'scanning'
                                                ? 'text-amber-700'
                                                : 'text-gray-500'
                                ...
                                >
                                    {scannerState === 'error'
                                        ? scannerResult?.message || 'Tap member gagal diproses.'
                                        : scannerState === 'success-masuk'
                                          ? `${scannerResult?.nama_member || 'Member'} tercatat masuk.`
                                          : scannerState === 'success-keluar'
                                            ? `${scannerResult?.nama_member || 'Member'} tercatat keluar.`
                                            : scannerState === 'scanning'
                                              ? 'Membaca UID dari NFC reader…'
                                              : 'Siap menerima scan kartu member.'}
                                </div>
                            </div>

                            <div className="rounded-2xl bg-gray-900 px-4 py-4 text-white flex flex-col justify-between min-h-[132px]">
                                <div className="text-[11px] uppercase tracking-[0.18em] text-gray-400 font-semibold">Hasil Terakhir</div>
                                {scannerResult?.ok ? (
                                    <>
                                        <div className="text-sm font-semibold leading-snug mt-2 line-clamp-2">
                                            {scannerResult?.nama_member || 'Member'}
                                        </div>
                                        <div className="flex items-center gap-2 mt-3 text-xs text-gray-300">
                                            <CheckCircle2 size={13} className="text-green-400 shrink-0" />
                                            {scannerResult?.aksi === 'keluar' ? 'Tap keluar berhasil' : 'Tap masuk berhasil'}
                                        </div>
                                    </>
                                ) : scannerResult?.uid ? (
                                    <>
                                        <div className="text-sm font-semibold leading-snug mt-2 text-red-300 line-clamp-2">
                                            {scannerResult?.message || 'Tap gagal'}
                                        </div>
                                        <div className="text-xs text-gray-400 mt-3 font-mono break-all">{scannerResult?.uid}</div>
                                    </>
                                ) : (
                                    <div className="text-sm text-gray-400 mt-2 leading-relaxed">
                                        Belum ada scan masuk pada sesi halaman ini.
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                        <div className="flex items-start justify-between gap-3 mb-3">
                            <div>
                                <h3 className="text-base font-bold text-gray-800 mb-1">Input Pengunjung Biasa</h3>
                                <p className="text-xs text-gray-500 leading-relaxed">
                                    Gunakan bila pengunjung tidak memiliki keychain NFC.
                                </p>
                            </div>
                            {activeEventId && (
                                <div className="text-[11px] px-2.5 py-1 rounded-full bg-green-50 text-green-700 border border-green-200 font-semibold">
                                    {namaEvent || 'Event aktif'}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                ...
                                className="w-full bg-green-500 hover:bg-green-600 disabled:bg-green-300 disabled:cursor-not-allowed text-white rounded-2xl px-4 py-4 flex flex-col items-center justify-center transition-all shadow-lg shadow-green-100 group"
                            >
                                <ArrowDownCircle size={28} className={`mb-2 ${!isManualButtonDisabled ? 'group-hover:scale-110' : ''} transition-transform`} />
                            ...
                            <button
                                ...
                                disabled={isManualButtonDisabled}
                                title={!activeEventId ? 'Menunggu data event aktif...' : ''}
                                className="w-full bg-red-500 hover:bg-red-600 disabled:bg-red-300 disabled:cursor-not-allowed text-white rounded-2xl px-4 py-4 flex flex-col items-center justify-center transition-all shadow-lg shadow-red-100 group"
                            >
                                <ArrowUpCircle size={28} className={`mb-2 ${!isManualButtonDisabled ? 'group-hover:scale-110' : ''} transition-transform`} />
                                <span className="text-base font-bold tracking-wide leading-none">
                ...
                <div className="min-w-0">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden h-full flex flex-col lg:h-[calc(100vh-340px)] min-h-[420px]">
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="text-sm divide-y divide-gray-50">
                                    {isLoadingActivities ? (
                                        ...
                                        activities.map((act) => (
                                            <tr key={`${act.id}-${flashKey}`} className="hover:bg-gray-50 transition row-flash align-top">
